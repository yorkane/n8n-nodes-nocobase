        ...
        this.methods = {
            listSearch: {
                    ...
                    try {
                        ...
                        if (typeof rawResponse === 'string') {
                            try {
                                ...
                                this.logger.error(`[NocoBase Node] listCollections: Failed to parse string response: ${e.message}`, { rawResponse });
                                return { results: [] };
                            }
                        }
                        else {
                        ...
                        const filteredCollections = collectionsSource.filter((collection) => {
                        ...
                        const results = filteredCollections.map((collection) => {
                        ...
                        this.logger.error(`[NocoBase Node] listCollections: Error fetching or processing collections: ${error.message}`, { error });
                        return { results: [] };
                    }
                },
                async listWorkflows() {
                    ...
                    try {
                        ...
                        if (rawOutputFromHelper && typeof rawOutputFromHelper === 'object' && typeof rawOutputFromHelper.response === 'string') {
                            try {
                                ...
                                this.logger.error('[NocoBase Node] listWorkflows: Path A - Failed to parse JSON from rawOutputFromHelper.response.', { error: e.message, responseString: rawOutputFromHelper.response });
                                return { results: [] };
                            }
                        }
                            ...
                            try {
                                ...
                                this.logger.error('[NocoBase Node] listWorkflows: Path C - Failed to parse direct string rawOutputFromHelper.', { error: e.message, rawOutputFromHelper });
                                return { results: [] };
                            }
                        }
                        else {
                        ...
                        const filteredWorkflows = workflowsSource.filter((wf) => {
                            return wf.enabled === true;
                        });
                        const results = filteredWorkflows.map((wf) => {
                        ...
                        if (!(error instanceof n8n_workflow_1.NodeOperationError)) {
                            this.logger.error(`[NocoBase Node] listWorkflows: Unhandled error in listWorkflows: ${error.message}`, { errorDetails: JSON.stringify(error, Object.getOwnPropertyNames(error)) });
                        }
                        return { results: [] };
                    }
                },
                async listRecords(filter) {
                    ...
                    const searchField = (_a = this.getNodeParameter('searchField')) !== null && _a !== void 0 ? _a : 'name';
                    const allRecords = [];
                    let page = 1;
                    const pageSize = 20;
                    ...
                    while (hasMore && allRecords.length < maxRecords) {
                        ...
                        try {
                            ...
                            this.logger.error(`[NocoBase Node] listRecords: Error fetching page ${page}: ${error.message}`);
                            break;
                        }
                    }
        ...
        for (let i = 0; i < items.length; i++) {
            try {
                ...
                if (['list', 'get', 'create', 'update', 'delete', 'move', 'select'].includes(operation)) {
                    ...
                    responseData = await workflowHandlers_1.handleExecuteWorkflowOperation.call(this, i, baseUrl, token);
                }
                else if (operation === 'getServerInfo') {
                    responseData = await otherHandlers_1.handleGetServerInfo.call(this, baseUrl, token);
                }
                else if (operation === 'listCustom') {
                    responseData = await otherHandlers_1.handleListCustomCollections.call(this, baseUrl, token);
                }
                else if (operation === 'listUsers') {
                    responseData = await otherHandlers_1.handleListUsers.call(this, baseUrl, token);
                }
                else if (operation === 'listWorkflows') {
                    responseData = await otherHandlers_1.handleListWorkflows.call(this, baseUrl, token);
                }
                else {
                    throw new n8n_workflow_1.NodeOperationError(this.getNode(), `The operation '${operation}' is not supported or implemented yet.`, { itemIndex: i });
                }
                ...
                if (typeof responseData === 'string') {
                    try {
                        ...
                        this.logger.error(`[NocoBase Node] Failed to parse JSON string response for operation '${operation}'. Raw string: "${responseData}". Error: ${parseError.message}`, { itemIndex: i });
                        throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Received a string response that could not be parsed as JSON for operation '${operation}': ${parseError.message}`, { itemIndex: i });
                    }
                }
